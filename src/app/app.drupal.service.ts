...
export class DrupalService {
  drupalUrl = "https://researchit.cer.auckland.ac.nz:8080/api/content";
  vid_service = 2;
  vid_lifecycle = 3;
  vid_policy = 4;
  vid_prog = 5;
  ...
  getProducts(taxonomyId)
  {
    ...
    return this.http
      ...
      .map((response) => this.getFilteredProducts(response, taxonomyId));
  }

  getFilteredProducts(response, taxonomyId)
  {
    ...
    for(let i = 1; i < data.length; i++)
    {
      ...
      if(product && DrupalService.productContainsTaxonomy(product, taxonomyId))
  ...
  {
    for(let i = 0; i < product.taxonomy.length; i++)
    {
      let taxonomy = product.taxonomy[i];

      if(taxonomy.tid === taxonomyId)
        return true;
    }
