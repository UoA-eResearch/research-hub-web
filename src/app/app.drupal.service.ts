...
export class DrupalService {
 ...
 contentsearch(category:string, searchChange:Subject<any>, debounceDuration = 400) {
  ...
  frontsearch(searchChange:Subject<any>, debounceDuration = 400) {
    return searchChange
      .debounceTime(debounceDuration)
      .distinctUntilChanged()
      .switchMap(value => this.rawFrontSearch(value.searchTerm, value.subcategories));
  }   
  rawSearch(category:string, term:string, subcategories:any[]) {
    ...
    return this.http
    .get(this.thisUrl + "?limit=10000" , {search:dosearch, headers:doheaders})
  ...
  rawFrontSearch(term:string, subcategories:any[]) {
    var dosearch = new URLSearchParams();
      console.log(dosearch);
    if (term != undefined && term.trim() != "") {
      dosearch.set('search_string', term);
